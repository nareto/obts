...
The migration preserves REST/MCP operation names, request/response schemas, ACL behavior, revision conflicts, and existing `503` fail-closed semantics. Operators must rebuild projection from verified headless state after changing projection/index settings; the rebuild must preserve retained access/audit history and must not replace `.obts` or visible files.
