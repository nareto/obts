...
- Capable clients finalize uploads asynchronously and poll durable processing outcomes, so proxy timeouts cannot turn an accepted proposal into a re-upload. Ancestry-safe stale device refs are accepted, superseded targets are idempotent, and divergent same-device history becomes a protected dashboard conflict.
- Server tree validation uses batched Git metadata and clean disjoint merges remain object-level; semantic worktrees are reserved for overlapping content that requires content-aware validation.
