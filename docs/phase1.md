...
- Plugin-side `.obts/` state with `isomorphic-git`, device token storage, durable watcher paths, queue state, schema-versioned scan watermark and metadata/OID cache, explicit directory-intent state for empty folder creation/deletion, crash-safe directory-baseline repair journals, recovery bundles with file snapshots, text patches, local-only Git refs packs, and artifact checksums, local apply lock, apply journal, local commit creation, device-token metadata rehydration when `state.json` is lost, multipart push, multipart pull, safe apply, safe incomplete-journal replay with recovery blocking when replay is unsafe, explicit replace-local-with-server recovery, and explicit rebuild from current server `main`.
- One sync decision performs one coherent pre-sync snapshot, revalidates only queued/affected state around pull, and labels its single post-apply preservation snapshot as verification instead of repeating full-vault `Checking` passes. Watcher paths force content reads only for affected files; unchanged reliable metadata plus matching cached Git OIDs avoid rereading the rest. Idle metadata inventory is capped at a six-hour cadence, complete byte-level audit runs weekly or on explicit request, and both survive reload through `.obts/scan-state.json` and `.obts/scan-cache.json`. Folder tombstones prune pre-existing empty descendants deepest-first with non-recursive removals, so stale shells are not re-uploaded as local creations while concurrent new folders and files remain protected. Directory proposals preserve stable IDs, generations, base provenance, and delete-then-create ancestry across transport. The server merges them against each device's acknowledged directory snapshot, while opposing subtree operations become dashboard conflicts before canonical state advances.
...
- Uploads preflight authoritative server device/vault state before object planning, preserve permanent rejection codes, back off transient retries, persist immutable attempt plans and transfer IDs across reload, and pack only commits plus changed tree/blob objects relative to acknowledged bases. A lost finalization response is recovered from the existing transfer descriptor before any rescan; large deletion packs therefore scale with retained/changed structure rather than deleted content history. A stale directory-proposal baseline enters `Repairing baseline`: a durable journal protects the rejected transfer, refs, cursors, queued commit, and original intent generations while the client proves ancestry, retrieves and acknowledges the historical authoritative directory snapshot already present locally, rebuilds effective intents, and creates a fresh proposal without writing visible files.
- Capable clients finalize uploads asynchronously and poll durable processing outcomes, so proxy timeouts cannot turn an accepted proposal into a re-upload. Internal Git or processing failures retain the proposal, retry with bounded backoff across restarts, and surface as `Server retrying` rather than unsafe local state. Ancestry-safe stale device refs are accepted, superseded targets are idempotent, and divergent same-device history becomes a protected dashboard conflict.
- Server tree validation uses batched Git metadata and clean disjoint merges remain object-level through a production-Git-2.39-compatible temporary index (`read-tree`, raw `diff-tree`, `update-index --index-info`, and `write-tree`); semantic worktrees are reserved for overlapping content that requires content-aware validation.
...
- legacy `.obts/directory-recovery.json` state is private migration evidence: the plugin archives and resumes it automatically, discards absent superseded shells, routes existing ambiguous directories to server-side dashboard review, revalidates identities and file fingerprints, and never exposes a local semantic decision flow;
- metadata schema v6 persists directory proposal outcomes and each device's acknowledged intent generations alongside its acknowledged event cursor and explicit-directory snapshot; migrations preserve legacy conflicts as content conflicts;
