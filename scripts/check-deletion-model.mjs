...
const manifestPath = resolve(process.env.FM004_CHECKS_MANIFEST ?? join(modelDir, 'checks-fm004.json'));
const architectureManifestPath = join(root, 'architecture', 'manifest.yaml');
const validateOnly = process.argv.includes('--validate-only');
const testMode = process.env.FM004_TEST_MODE === '1';
const runRoot = mkdtempSync(join(tmpdir(), 'obts-fm004-corrected-'));
const requiredChecks = [
  ['fm004-deletion-safety', 'positive-safety'], ['fm004-deletion-liveness', 'positive-liveness'],
  ['fm004-blocked-integrity-safety', 'positive-safety'], ['fm004-receipt-expiry-liveness', 'positive-liveness'],
  ['fm004-reach-crash-restart', 'reachability'], ['fm004-reach-erase-fault', 'reachability'],
  ['fm004-reach-final-fault', 'reachability'], ['fm004-reach-final-fault-crash', 'reachability'],
  ['fm004-reach-receipt-expiry', 'reachability'], ['fm004-reach-blocked-target', 'reachability'],
  ['fm004-reach-preintent-crash', 'reachability'], ['fm004-reach-intent-failure', 'reachability'],
  ['fm004-reach-response-acceptance', 'reachability'],
  ['fm004-negative-intent-failure-acceptance', 'negative-control'],
  ['fm004-negative-response-before-intent', 'negative-control'],
  ['fm004-negative-wrong-confirmation', 'negative-control'],
  ['fm004-negative-stale-live-selection', 'negative-control'],
  ['fm004-negative-wrong-owner', 'negative-control'], ['fm004-negative-unknown-target', 'negative-control'],
  ['fm004-negative-erase-before-intent', 'negative-control'],
  ['fm004-negative-admission-release-race', 'negative-control'],
  ['fm004-negative-premature-completion', 'negative-control'],
  ['fm004-negative-expire-unfinished', 'negative-control'],
  ['fm004-negative-early-release-late-work', 'negative-control'],
  ['fm004-negative-unattributed-residue', 'negative-control'],
  ['fm004-negative-startup-recreate', 'negative-control'],
  ['fm004-negative-boundary-mutation', 'negative-control'],
  ['fm004-negative-other-vault', 'negative-control'],
  ['fm004-negative-ambiguous-reopen', 'negative-control']
];

try {
  const manifest = loadJson(manifestPath, 'FM004 checks manifest');
  validateManifest(manifest);
  if (validateOnly) {
    console.log(`FM004 corrected candidate manifest valid: ${manifest.checks.length} required checks; bounds/configs valid.`);
  } else {
    runSany(manifest);
    const summaries = manifest.checks.map((check) => runCheck(manifest, check));
    console.log(`FM004 corrected candidate passed: ${summaries.length} checks; ${summaries.filter(x => x.outcome === 'PASS').length} positive, ${summaries.filter(x => x.outcome === 'REACHED').length} reachability/negative controls.`);
  ...
  console.error(`FM004 corrected candidate failed: ${error.message}`);
  process.exitCode = 1;
} finally {
...
function validateManifest(manifest) {
  if (manifest.schemaVersion !== 1 || manifest.modelId !== 'OBTS-FM-004' || manifest.architectureRevision !== 7 || !['candidate', 'accepted'].includes(manifest.architectureStatus)) throw new Error('candidate checks manifest metadata is invalid.');
  const arch = readFileSync(architectureManifestPath, 'utf8');
  if (!/- id: OBTS-FM-004[\s\S]*?\n\s+status:\s*(candidate|accepted)\b[\s\S]*?\n\s+architecture_revision:\s*7\b/u.test(arch)) throw new Error('architecture inventory lacks FM004 revision 7.');
  if (manifest.model !== 'OBTSVaultDeletion.tla' || !Array.isArray(manifest.sanyModules) || (!testMode && (manifest.sanyModules.length !== 1 || manifest.sanyModules[0] !== 'OBTSVaultDeletion'))) throw new Error('model/SANY declaration is invalid.');
  ...
  if (t.workers !== 1 || t.fingerprintPolynomial !== 0) throw new Error('corrected FM004 requires one worker and fingerprint polynomial 0.');
  requireFile(containedModelPath(manifest.model, 'model'), 'FM004 model');
  ...
  if (!testMode) {
    if (manifest.checks.length !== requiredChecks.length) throw new Error(`corrected FM004 requires exactly ${requiredChecks.length} checks.`);
  ...
  for (const check of manifest.checks) {
    ...
    if (check.maximumDistinctStates >= 30000) throw new Error(`${check.id} exceeds corrected bounded-state budget.`);
...
function runTool(manifest, tool, args, fixtureId, timeoutMs = manifest.tooling.defaultTimeoutMs, heapMb = manifest.tooling.defaultHeapMb) {
  const fixtureDir = process.env.FM004_FIXTURE_DIR;
  if (fixtureDir) {
  ...
  const evidenceDir = process.env.FM004_EVIDENCE_DIR;
  if (evidenceDir) {
