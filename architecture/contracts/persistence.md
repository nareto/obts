...
Transfer availability and transfer readiness are separate. Serving availability is suspended only by genuine storage conditions: an unusable transfer root, a durable state write that failed, or uncertain Git durability. A session-scoped inconsistency in pre-existing state (an unreadable, mismatched, oversized, or accounting-drifted session, or a quarantine repository that fails integrity validation) is reported as a readiness anomaly and refused for that transfer, and never disables transfers for other devices or vaults. Readiness inspection is read-only: it writes nothing and cannot itself suspend serving. Deletion-scoped inventory keeps failing closed on unattributable residue regardless of this serving behavior.
