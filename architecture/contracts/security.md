...
A consumed connection's authenticated status may return its original selection, approval baseline, and accepted mode for enrollment recovery. This receipt and completion replay remain bound to the original connection, owner, vault and device; disabled owners, revoked or rotated credentials, and deleted vaults cannot recover enrollment access. The receipt never substitutes the current canonical head for the approval baseline.
...
Bridge headless progress is a liveness signal, not authority. Startup and operation progress events use a closed, byte-bounded schema containing only redacted status and diagnostic-point identifiers, never paths, manifests, content, tokens, secrets, or browser URLs. Only schema-valid correlated responses, at most one closed-schema state event per request, and closed-schema progress events reset the configured inactivity timer; missing, extra, oversized, invalid, or unrelated traffic fails the protocol and quarantines the child. Request writes, startup recovery, complete stdout frames, and response reads are all bounded. Cancellation synchronously records the cancelled command, marks the child unhealthy, and starts quarantine before releasing request ownership. The bounded restart circuit pauses automatic restarts for a configured cooldown, then attempts supervised recovery; success clears the circuit and the failure window, while persistent failure keeps the pause and degrades readiness. Progress cannot extend enrollment authority or mutate durable transfer state.
...
Troubleshooting reads at most 512 KiB per coordination JSON file. Larger journals may use a separately published, freshness-checked compact observation derived during normal validated journal reads or publication. Summaries contain only fixed phase/error/consent/checkpoint categories, never file paths, names, contents, hashes, credentials or raw errors. Invalid, unreadable and oversized reads remain distinguishable. Summaries are optional diagnostic observations and never recovery authority; existing diagnostic clients remain accepted.
