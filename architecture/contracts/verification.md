...
- Onboarding receipt/recovery evidence includes a usable packaged modal from early disposition through browser approval, failure, discarded in-memory state and Resume; historical null-analysis journals with authentic original consent; rejection of stale/missing consent and identity/mode/revocation mismatches; actual process termination at separate enrollment, credential, identity, marker, chunk, recovery, apply, acknowledgement and activation publications; exact recovered bytes/directories and no duplicate registration/proposal. Complete-checkpoint and pending-ack tests advance canonical main between attempts. Foreground/manual retries exercise both event orders, close/unload and exclusive ownership. Diagnostics cover the 512 KiB boundary, oversized compact observations, redaction and old payload acceptance.
