...
- Bridge whole-file revision evidence for both note and raw-file surfaces: exact reads and create/update receipts return the opaque raw-file revision; missing and stale preconditions produce 428/412; matching writes produce a new revision; unknown fields are rejected; an external change between preparation and atomic replacement leaves source and projection unchanged.
