...
`OBTS-FM-002` is the candidate composed distributed refinement of `OBTS-SAF-001` through `OBTS-SAF-006` and the named synchronization, persistence, and Bridge obligations. Candidate status records unresolved implementation-faithful counterexamples rather than weakening these properties. Its exact bounds, assumptions, gates, transition mapping, and discrepancy evidence live in `architecture/models/formal/README.md` and ADR 0008.
