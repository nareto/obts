...
`OBTS-FM-002` is the accepted composed distributed refinement of `OBTS-SAF-001` through `OBTS-SAF-006` and the named synchronization, persistence, and Bridge obligations. Its exact bounds, assumptions, gates, transition mapping, and implementation-recovery regression live in `architecture/models/formal/README.md` and ADR 0008.

`OBTS-FM-003` is the accepted architecture-stage refinement of Bridge row lifecycle and ownership. Independent required rows/kinds/sizes, normal-or-singleton shape, retained shared singleton ownership, cancellation/drain, partial commit/restart/replay and completion-after-all-rows/cleanup replace the inadequate one-row-per-file abstraction. Hard input/normal-batch and ownership guarantees do not imply a numerical encoded-singleton ceiling or RSS bound. SQL/compiler, lexical equivalence, API format/order, parser behavior and production conformance require implementation/deployment evidence. See ADR 0009 and the formal trace map for corrected claims and reviewed implementation gaps.
