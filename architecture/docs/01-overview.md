...
The Bridge is part of the current architecture. Rust exposes scoped REST/MCP tools, ACL/revision enforcement, ordinary visible-file mutation, and rebuildable PostgreSQL projections. A supervised Node process runs the same client core as the plugin and owns pairing, hidden Git, queues, synchronization, apply, and recovery.

The persistent headless visible vault and `.obts/` directory are authoritative device state and may contain the only copy of pending agent work. PostgreSQL may be rebuilt from verified headless state; it must never replace uncertain client state. The Bridge pairs and synchronizes as a normal protected OBTS device.
...
TLA+/PlusCal remains a bounded optional refinement under `architecture/models/formal/`; no formal model is currently accepted.
