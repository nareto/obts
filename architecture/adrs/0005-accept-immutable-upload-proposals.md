...
The server separates durable receipt from canonical integration. After all chunks are present, capable clients request asynchronous finalization and receive a processing descriptor immediately. The client polls the transfer resource. Processing and terminal results remain in the transfer session, and polling restarts interrupted processing after a server restart. Internal Git or integration failures keep the proposal in processing with bounded backoff and a safe retry classification; they are not terminal proposal rejections.
...
Tree path and size validation uses one batched `git ls-tree -r -l -z` operation. Clean disjoint merge uses an explicit-base temporary index with `git read-tree`, raw `git diff-tree`, `git update-index --index-info`, `git write-tree`, and `git commit-tree`, which is supported by the production Git 2.39 baseline and does not materialize a worktree. Temporary plaintext worktrees remain limited to overlapping content requiring semantic validation.
...
- Transfer descriptors gain `processing` and `rejected` terminal semantics, advertise a polling interval, and may expose a safe processing-retry classification.
