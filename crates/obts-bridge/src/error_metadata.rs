...
pub(crate) fn service_error_metadata(
    ...
    let mut metadata = match error {
        ...
        ServiceError::Forbidden => ErrorMetadata::new(
            ErrorCategory::Permission,
            false,
            "operation forbidden",
            "This operation requires the admin authorization context",
        )
        .with_http_status(403),
        ServiceError::HeadlessNotPaired => ErrorMetadata::new(
            ErrorCategory::Business,
            false,
            "headless client not paired",
            "Pair the Titania headless client through the authenticated administration endpoint before using vault tools",
        )
        .with_http_status(503),
        ServiceError::IndexCatchingUp => ErrorMetadata::new(
        ...
        ServiceError::FilesystemWrite(error) => match error {
            ...
            _ => ErrorMetadata::new(
                ErrorCategory::Transient,
                true,
                "vault filesystem unavailable",
                "The authorized change could not be persisted to the headless vault filesystem",
            )
            .with_http_status(503),
        },
