...
fn service_error_legacy_error(error: &ServiceError) -> String {
    match error {
        ...
        ServiceError::HeadlessNotPaired => "headless OBTS client is not paired".to_string(),
        ServiceError::IndexCatchingUp => "vault index is catching up".to_string(),
...
struct HeadlessCommandRequest {
    command: String,
    #[serde(default)]
    arguments: Value,
}

async fn headless_command(
    State(state): State<AppState>,
    headers: HeaderMap,
    payload: Result<Json<HeadlessCommandRequest>, JsonRejection>,
) -> Result<Json<Value>, ApiError> {
    let auth = authorize_from_headers(&state, &headers).await?;
    let request = decode_json(payload)?;
    let result = state
        .service
        .headless_command(&auth, request.command.trim(), request.arguments)
        .await?;
    Ok(Json(result))
}
...
pub fn app_router(state: AppState) -> Router {
    ...
    let mut router = Router::new()
        ...
        .route("/api/v1/admin/headless/command", post(headless_command))
...
fn render_prometheus_metrics(status: &StatusResponse) -> String {
    let image_tag = std::env::var("OBTS_BRIDGE_IMAGE_TAG").ok();
    let git_commit = std::env::var("OBTS_BRIDGE_GIT_COMMIT").ok();
    let mut lines = vec![
        "# HELP obts_bridge_build_info Build and deployment metadata for the OBTS Bridge API process.".to_string(),
        "# TYPE obts_bridge_build_info gauge".to_string(),
        build_info_metric_line(
            env!("CARGO_PKG_VERSION"),
            image_tag.as_deref(),
            git_commit.as_deref(),
        ),
        "# HELP obts_bridge_dependency_up Whether a required service dependency is available.".to_string(),
        "# TYPE obts_bridge_dependency_up gauge".to_string(),
        format!(
            "obts_bridge_dependency_up{{dependency=\"postgres\"}} {}",
            usize::from(status.dependencies.postgres != "unavailable")
        ),
        format!(
            "obts_bridge_dependency_up{{dependency=\"couchdb\"}} {}",
            usize::from(status.dependencies.couchdb != "unavailable")
        ),
        format!(
            "obts_bridge_dependency_up{{dependency=\"obts_client\"}} {}",
            usize::from(status.dependencies.obts_client == "healthy")
        ),
        format!(
        ...
        format!(
            "obts_bridge_pending_write_projections {}",
            status.write_projection.pending
        ),
        "# HELP obts_bridge_sync_behind_by CouchDB change-sequence lag.".to_string(),
        "# TYPE obts_bridge_sync_behind_by gauge".to_string(),
        format!("obts_bridge_sync_behind_by {}", status.sync.behind_by),
        "# HELP obts_bridge_sync_current_sequence_live Whether sync lag uses a live CouchDB watermark.".to_string(),
        "# TYPE obts_bridge_sync_current_sequence_live gauge".to_string(),
        format!(
            "obts_bridge_sync_current_sequence_live {}",
            usize::from(status.sync.current_seq_source == "live")
        ),
        "# HELP obts_bridge_pending_sync_recoveries Unresolved sync recovery targets waiting for retry.".to_string(),
        "# TYPE obts_bridge_pending_sync_recoveries gauge".to_string(),
        format!(
            "obts_bridge_pending_sync_recoveries {}",
            status.index.pending_sync_recoveries
        ),
        "# HELP obts_bridge_quarantined_sync_recoveries Sync recovery targets quarantined after repeated failures.".to_string(),
        "# TYPE obts_bridge_quarantined_sync_recoveries gauge".to_string(),
        format!(
            "obts_bridge_quarantined_sync_recoveries {}",
            status.index.quarantined_sync_recoveries
        ),
        "# HELP obts_bridge_pending_embeddings Notes missing embeddings.".to_string(),
        "# TYPE obts_bridge_pending_embeddings gauge".to_string(),
        format!(
            "obts_bridge_pending_embeddings {}",
            status.index.pending_embeddings
        ),
        "# HELP obts_bridge_quarantined_embeddings Notes that exceeded max embedding failures."
            .to_string(),
        "# TYPE obts_bridge_quarantined_embeddings gauge".to_string(),
        format!(
            "obts_bridge_quarantined_embeddings {}",
            status.index.quarantined_embeddings
        ),
        "# HELP obts_bridge_pending_chunk_embeddings Semantic chunks missing embeddings."
            .to_string(),
        "# TYPE obts_bridge_pending_chunk_embeddings gauge".to_string(),
        format!(
            "obts_bridge_pending_chunk_embeddings {}",
            status.index.pending_chunk_embeddings
        ),
        "# HELP obts_bridge_quarantined_chunk_embeddings Semantic chunks that exceeded max embedding failures."
            .to_string(),
        "# TYPE obts_bridge_quarantined_chunk_embeddings gauge".to_string(),
        format!(
            "obts_bridge_quarantined_chunk_embeddings {}",
            status.index.quarantined_chunk_embeddings
        ),
        "# HELP obts_bridge_embedding_backend_degraded Embedding backend degraded state."
            .to_string(),
        "# TYPE obts_bridge_embedding_backend_degraded gauge".to_string(),
        format!(
            "obts_bridge_embedding_backend_degraded {}",
            usize::from(status.embedding.backend_state == "degraded")
        ),
        "# HELP obts_bridge_embedding_last_success_timestamp_seconds Last embedding provider success timestamp."
            .to_string(),
        "# TYPE obts_bridge_embedding_last_success_timestamp_seconds gauge".to_string(),
        format!(
            "obts_bridge_embedding_last_success_timestamp_seconds {}",
            status
                .embedding
                .last_success_at
        ...
        "# HELP obts_bridge_embedding_last_error_timestamp_seconds Last embedding provider or payload error timestamp."
            .to_string(),
        "# TYPE obts_bridge_embedding_last_error_timestamp_seconds gauge".to_string(),
        format!(
            "obts_bridge_embedding_last_error_timestamp_seconds {}",
            status
                .embedding
                .last_error_at
                .map(|timestamp| timestamp.timestamp())
        ...
        "# HELP obts_bridge_pending_chunks Parents waiting for full chunk sets.".to_string(),
        "# TYPE obts_bridge_pending_chunks gauge".to_string(),
        format!(
            "obts_bridge_pending_chunks {}",
            status.index.pending_chunks
        ),
        "# HELP obts_bridge_orphan_leaf_staging Parents in chunk staging that are bare LiveSync leaf ids without a current file alias.".to_string(),
        "# TYPE obts_bridge_orphan_leaf_staging gauge".to_string(),
        format!(
            "obts_bridge_orphan_leaf_staging {}",
            status.index.orphan_leaf_staging_count
        ),
        "# HELP obts_bridge_stale_file_aliases LiveSync file aliases whose raw file or Markdown index row is missing or stale.".to_string(),
        "# TYPE obts_bridge_stale_file_aliases gauge".to_string(),
        format!(
            "obts_bridge_stale_file_aliases {}",
            status.index.stale_file_aliases
        ),
        "# HELP obts_bridge_stale_aliases_blocked_unavailable_children Stale LiveSync aliases diagnosed with missing or tombstoned children.".to_string(),
        "# TYPE obts_bridge_stale_aliases_blocked_unavailable_children gauge".to_string(),
        format!(
            "obts_bridge_stale_aliases_blocked_unavailable_children {}",
            status.index.stale_aliases_blocked_by_unavailable_children
        ),
        "# HELP obts_bridge_unavailable_livesync_children LiveSync child documents unavailable to note assembly by source state.".to_string(),
        "# TYPE obts_bridge_unavailable_livesync_children gauge".to_string(),
        format!(
            "obts_bridge_unavailable_livesync_children{{state=\"missing\"}} {}",
            status.index.missing_livesync_children
        ),
        format!(
            "obts_bridge_unavailable_livesync_children{{state=\"tombstoned\"}} {}",
            status.index.tombstoned_livesync_children
        ),
        "# HELP obts_bridge_missing_vault_files_for_notes Indexed Markdown notes without a matching raw vault file.".to_string(),
        "# TYPE obts_bridge_missing_vault_files_for_notes gauge".to_string(),
        format!(
            "obts_bridge_missing_vault_files_for_notes {}",
            status.index.missing_vault_files_for_notes
        ),
        "# HELP obts_bridge_unindexed_markdown_vault_files Raw Markdown vault files without a matching indexed note.".to_string(),
        "# TYPE obts_bridge_unindexed_markdown_vault_files gauge".to_string(),
        format!(
            "obts_bridge_unindexed_markdown_vault_files {}",
            status.index.unindexed_markdown_vault_files
        ),
        "# HELP obts_bridge_total_notes Total indexed notes.".to_string(),
        "# TYPE obts_bridge_total_notes gauge".to_string(),
        format!("obts_bridge_total_notes {}", status.index.total_notes),
        "# HELP obts_bridge_total_links Total indexed links.".to_string(),
        "# TYPE obts_bridge_total_links gauge".to_string(),
        format!("obts_bridge_total_links {}", status.index.total_links),
        "# HELP obts_bridge_total_tags Total unique tags.".to_string(),
        "# TYPE obts_bridge_total_tags gauge".to_string(),
        format!("obts_bridge_total_tags {}", status.index.total_tags),
        "# HELP obts_bridge_config_reload_enabled Config hot reload enabled state.".to_string(),
        "# TYPE obts_bridge_config_reload_enabled gauge".to_string(),
        format!(
            "obts_bridge_config_reload_enabled {}",
            usize::from(status.config_reload.enabled)
        ),
        "# HELP obts_bridge_config_reload_generation Active validated config generation.".to_string(),
        "# TYPE obts_bridge_config_reload_generation gauge".to_string(),
        format!(
            "obts_bridge_config_reload_generation {}",
            status.config_reload.generation
        ),
        "# HELP obts_bridge_config_reload_success_total Successful config reload attempts."
            .to_string(),
        "# TYPE obts_bridge_config_reload_success_total counter".to_string(),
        format!(
            "obts_bridge_config_reload_success_total {}",
            status.config_reload.success_count
        ),
        "# HELP obts_bridge_config_reload_failure_total Failed config reload attempts.".to_string(),
        "# TYPE obts_bridge_config_reload_failure_total counter".to_string(),
        format!(
            "obts_bridge_config_reload_failure_total {}",
            status.config_reload.failure_count
        ),
        "# HELP obts_bridge_config_reload_last_success_timestamp_seconds Last successful config load or reload timestamp.".to_string(),
        "# TYPE obts_bridge_config_reload_last_success_timestamp_seconds gauge".to_string(),
        format!(
            "obts_bridge_config_reload_last_success_timestamp_seconds {}",
            status
                .config_reload
                .last_success_at
                .map(|timestamp| timestamp.timestamp())
        ...
        "# HELP obts_bridge_config_reload_last_failure_timestamp_seconds Last failed config reload timestamp.".to_string(),
        "# TYPE obts_bridge_config_reload_last_failure_timestamp_seconds gauge".to_string(),
        format!(
            "obts_bridge_config_reload_last_failure_timestamp_seconds {}",
            status
                .config_reload
                .last_failure_at
                .map(|timestamp| timestamp.timestamp())
    ...
    for (context, stats) in contexts {
        lines.push(format!(
            "obts_bridge_accessible_notes{{context=\"{}\"}} {}",
            context, stats.accessible_notes
        ));
        lines.push(format!(
            "obts_bridge_filtered_notes{{context=\"{}\"}} {}",
...
fn build_info_metric_line(
    ...
    format!(
        "obts_bridge_build_info{{version=\"{}\",image_tag=\"{}\",commit=\"{}\"}} 1",
...
mod tests {
    ...
    fn build_info_metric_derives_commit_from_sha_image_tag() {
        assert_eq!(
            ...
            r#"obts_bridge_build_info{version="0.1.0",image_tag="sha-1f0ad51",commit="1f0ad51"} 1"#
        );
    }
    ...
    fn build_info_metric_prefers_explicit_git_commit() {
        assert_eq!(
            ...
            r#"obts_bridge_build_info{version="0.1.0",image_tag="sha-1f0ad51",commit="1f0ad51fdd49f437cb61a09ab14cbb2d4c508dde"} 1"#
        );
    }
    ...
    fn build_info_metric_uses_unknown_for_unset_deploy_metadata() {
        assert_eq!(
            ...
            r#"obts_bridge_build_info{version="0.1.0",image_tag="unknown",commit="unknown"} 1"#
        );
    }
