...
impl AppConfig {
    ...
    fn apply_legacy_env_overrides(&mut self) -> Result<(), ConfigError> {
        if let Ok(host) = env::var("OBTS_BRIDGE_HOST")
            && !host.trim().is_empty()
        {
            self.server.host = host;
        }

        if let Ok(port) = env::var("OBTS_BRIDGE_PORT") {
...
impl Default for ClientConfig {
    fn default() -> Self {
        Self {
            vault_dir: "/var/lib/obts-bridge/vault".to_string(),
            headless_command: "node /app/obts/dist/src/headless.js".to_string(),
...
impl Default for DatabaseConfig {
    fn default() -> Self {
        Self {
            url: "postgres://obts_bridge:${PG_PASS}@localhost:5432/obts_bridge".to_string(),
            max_connections: 10,
        }
    }
}
...
pub enum ConfigError {
    ...
    #[error("OBTS_BRIDGE_PORT is invalid: '{0}'")]
    InvalidPort(String),
    #[error("invalid headless client configuration: {0}")]
